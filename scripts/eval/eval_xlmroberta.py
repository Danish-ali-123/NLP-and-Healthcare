...
def main() -> None:
    backbone_dir = PROJECT_ROOT / "experiments" / "xlmroberta"
    latest_run = find_latest_experiment(backbone_dir)
    
    print(f"📊 Found latest XLM-RoBERTa experiment: {latest_run}")
    
    for lang in ['en', 'hi', 'pa']:
        model_root_rel = f"experiments/xlmroberta/{latest_run}/{lang}"
        output_rel = f"reports/eval_supervised/xlmroberta/{latest_run}/{lang}"
        label = f"XLM-RoBERTa / {latest_run} / {lang}"
