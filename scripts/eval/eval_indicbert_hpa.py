...
def main() -> None:
    backbone_dir = PROJECT_ROOT / "experiments" / "indicbert_hpa"
    latest_run = find_latest_experiment(backbone_dir)
    
    print(f"📊 Found latest IndicBERT-HPA experiment: {latest_run}")
    
    # IndicBERT-HPA trains on all languages: en (IndicBERT+linear), hi/pa (IndicBERT+HPA)
    for lang in ['en', 'hi', 'pa']:
        model_root_rel = f"experiments/indicbert_hpa/{latest_run}/{lang}"
        output_rel = f"reports/eval_supervised/indicbert_hpa/{latest_run}/{lang}"
        label = f"IndicBERT-HPA / {latest_run} / {lang}"
