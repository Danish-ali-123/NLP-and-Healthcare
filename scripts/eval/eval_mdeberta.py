...
def main() -> None:
    backbone_dir = PROJECT_ROOT / "experiments" / "mdeberta"
    latest_run = find_latest_experiment(backbone_dir)
    
    print(f"📊 Found latest mDeBERTa experiment: {latest_run}")
    
    for lang in ['en', 'hi', 'pa']:
        model_root_rel = f"experiments/mdeberta/{latest_run}/{lang}"
        output_rel = f"reports/eval_supervised/mdeberta/{latest_run}/{lang}"
        label = f"mDeBERTa / {latest_run} / {lang}"
