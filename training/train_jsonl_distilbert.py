...
# Import our dataset implementations
from train_jsonl.jsonl_dataset import JSONLClinicalDataset, DataCollator, load_label_mapping, create_jsonl_data_loaders
...
class CSVClinicalDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.labels[idx]
            # NEW: Final Clinical Masking at runtime to catch anything missed in CSV
        # This prevents the model from seeing "Hip", "Spine", or specific condition names
        # Catch the original words AND your new standardized zones
...
def predict_diagnosis(text_data: Dict, model, tokenizer, label_info, device, max_length=384):
    ...
    if 'text_input' in text_data:
        ...
        # REMOVE Diagnosis and Remarks to prevent cheating
    # NEW VERSION (Matches your new CSV exactly)
...
class Trainer:
    """Trainer for clinical NLP models with JSONL data."""
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # Update running metrics (Acc every step, F1 every 10 steps to maintain speed)
...
def build_model_config_from_config(
    config: Dict[str, Any],
    language: str
) -> Dict[str, Any]:
    """Build model config for a specific language from base config."""
    ...
    # Add head config with proper dropout
    if head_type == 'linear':
        dropout = lang_model_cfg.get('linear', {}).get('dropout', 0.1)
    else:
        dropout = lang_model_cfg.get(head_type, {}).get('dropout', 0.1)
...
def main():
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description='Train DistilBERT model on CSV or JSONL data')
    ...
    parser.add_argument('--jsonl_path', type=str, default=None,
                        help='Path to JSONL data file (optional, used if csv_path is not provided)')
    ...
    experiment_dir = os.path.join(args.output_dir, "distilbert")
    ...
    model_config = build_model_config_from_config(config, args.language)
    ...
    # We delay model building until after data loading if label_info is not available
    ...
    if args.csv_path and os.path.exists(args.csv_path):
        ...
        logger.info(f"Creating data loaders from JSONL: {args.jsonl_path}")
        data_loaders = create_jsonl_data_loaders(
            jsonl_path=args.jsonl_path,
            tokenizer=tokenizer,
            label2id=label_info['label2id'],
            batch_size=batch_size,
            max_length=max_seq_len,
            test_split=config['data']['split']['test_ratio'],
            val_split=config['data']['split']['val_ratio'],
    ...
    # The model is a MultilingualClassifier with an 'encoder' attribute
    if hasattr(model, 'encoder'):
        encoder = model.encoder
        logger.info(f"Found encoder attribute in MultilingualClassifier")
    ...
    warmup_steps = int(total_steps * config['training'].get('warmup_ratio', 0.05))
