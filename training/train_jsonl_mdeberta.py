...
class CSVClinicalDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Get text and label - use 'text_input' column which actually exists in the CSV
...
def build_mdeberta_model(
    model_config: Dict[str, Any],
    label_info: Dict[str, Any],
    device: torch.device
) -> nn.Module:
    """Build MDeBERTa model."""
...
class Trainer:
    ...
    def __init__(
        ...
        self.experiment_dir = os.path.join('train_jsonl/experiments', 'mdeberta')
...
def main():
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description='Train MDeBERTa model on CSV data')
    parser.add_argument('--config', type=str, default='src/config/proper_mdeberta.yaml',
    ...
    experiment_dir = os.path.join(args.output_dir, "mdeberta")
    ...
    logger.info(f"Building MDeBERTa model")
    
    # Build model using the custom function that applies dropout from config
    ...
    # Add dropout config if it exists
    if 'linear' in lang_model_cfg:
        model_config['model']['head'] = {
            'dropout': lang_model_cfg['linear'].get('dropout', 0.1)
        }
    ...
    csv_path = os.path.join(results_dir, f'mdeberta_{args.language}_metrics.csv')
