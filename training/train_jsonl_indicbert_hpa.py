...
def build_indicbert_hpa_model(
    ...
    hpa_config = model_config.get('model', {}).get('hpa', {})
...
def main():
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description='Train IndicBERT-HPA model on CSV data')
    ...
    experiment_dir = os.path.join(args.output_dir, "indicbert_hpa")
    ...
    if args.csv_path and os.path.exists(args.csv_path):
        ...
        # For English, only keep English labels
        # For Hindi, only keep Hindi labels
        # For Punjabi, only keep Punjabi labels
    ...
    logger.info(f"Building IndicBERT-HPA model")
    # Get language-specific configuration
    lang_model_config = config.get('models', {}).get(args.language, {})
    # Get HPA configuration
    hpa_config = lang_model_config.get('hpa', {})
    adapter_hidden_sizes = hpa_config.get('hidden_sizes', [512])
    adapter_dropout = hpa_config.get('dropout', 0.2)
    
    model = IndicBertHPAClassifier(
        backbone_checkpoint=model_config['model']['name'],
        num_labels=len(label_info['labels']),
        id2label={int(k): v for k, v in label_info['id2label'].items()},
        label2id=label_info['label2id'],
        adapter_hidden_sizes=adapter_hidden_sizes,
    ...
    # Set up scheduler with better parameters for generalization
    num_epochs = args.epochs if args.epochs is not None else config['training'].get('epochs', 30)
