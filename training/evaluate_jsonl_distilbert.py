...
def evaluate_model(
    ...
    metric_calculator = MetricCalculator(id2label_int)
    ...
    try:
        ...
        auroc_weighted = roc_auc_score(all_labels, all_probabilities, multi_class='ovr', average='weighted')
        metrics['auroc'] = {
            'macro': auroc_macro,
            'weighted': auroc_weighted
        }
        
        # AUPRC
        auprc_macro = average_precision_score(all_labels, all_probabilities, average='macro')
        auprc_weighted = average_precision_score(all_labels, all_probabilities, average='weighted')
        metrics['auprc'] = {
            'macro': auprc_macro,
            'weighted': auprc_weighted
        }
        
        logger.info(f"AUROC: macro={auroc_macro:.4f}, weighted={auroc_weighted:.4f}")
        logger.info(f"AUPRC: macro={auprc_macro:.4f}, weighted={auprc_weighted:.4f}")
